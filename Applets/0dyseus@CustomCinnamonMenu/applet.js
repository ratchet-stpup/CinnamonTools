...
MyApplet.prototype = {
	...
	_init: function(orientation, panel_height, instance_id) {
		...
		// try {
		// 	global.logError(global.settings.get_strv('favorite-apps'));
		// 	global.logError();
		// } catch (aErr) {}
	},
	...
	_refreshRecent: function() {
		...
		if (this.privacy_settings.get_boolean(REMEMBER_RECENT_KEY) &&
			...
			if (this.RecentManager._infosByTimestamp.length > 0) {
				for (let id = 0; id < this.pref_max_recent_files && id < this.RecentManager._infosByTimestamp.length; id++) {
	...
	_clearAllSelections: function(hide_apps) {
		...
		for (let i = 0; i < actors3.length; i++) {
			let actor = actors3[i];
