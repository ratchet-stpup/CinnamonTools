 ...
 * - Added icons to the window list button context menu.
 */
...
AppMenuButton.prototype = {
	_init: function(applet, metaWindow, alert) {
		...
		if (this._applet.pref_hide_labels)
			obj["style"] = 'padding-left: 0; padding-right: 0;';

		this.actor = new Cinnamon.GenericContainer(obj);
	...
	_getPreferredWidth: function(actor, forHeight, alloc) {
		...
		if (this._applet.pref_hide_labels)
			alloc.natural_size = alloc.min_size = naturalSize + 2 * 3 * global.ui_scale;
		// alloc.natural_size = alloc.min_size = naturalSize + 2 * 3 * global.ui_scale;
		else {
...
AppMenuButtonRightClickMenu.prototype = {
	...
	_populateMenu: function() {
		...
		let item;
		let length;
		item = new PopupMenu.PopupIconMenuItem(_("Configure Window list applet"), "system-run", St.IconType.SYMBOLIC);
		item.connect('activate', Lang.bind(this, function() {
			Util.spawnCommandLine("cinnamon-settings applets " + AppletUUID + " " + this._launcher._applet.instance_id);
		}));
		...
		// Close all/others
		item = new PopupMenu.PopupIconMenuItem(_("Close all"), "edit-delete", St.IconType.SYMBOLIC);
		item.connect('activate', Lang.bind(this, function() {
		...
		item = new PopupMenu.PopupIconMenuItem(_("Close others"), "edit-delete", St.IconType.SYMBOLIC);
		item.connect('activate', Lang.bind(this, function() {
			for (let window of this._windows)
				if (window.actor.visible &&
		...
		if (mw.get_compositor_private().opacity !== 255) {
			item = new PopupMenu.PopupIconMenuItem(_("Restore to full opacity"), "system-run", St.IconType.SYMBOLIC);
			item.connect('activate', function() {
...
MyApplet.prototype = {
	...
	_init: function(orientation, panel_height, instance_id) {
		...
		this.settings = new Settings.AppletSettings(this, AppletUUID, this.instance_id);
