...
function HotCornerManager(aDelayObj) {
	this._init(aDelayObj);
}

HotCornerManager.prototype = {
	_init: function(aDelayObj) {
		this.corners = [];
		for (let i = 0; i < 4; i++) { // In order: top left; top right; bottom left; bottom right;
			this.corners.push(new HotCorner(aDelayObj[i]));
...
// overview.
function HotCorner(aDelay) {
	this._init(aDelay);
}

HotCorner.prototype = {
	_init: function(aDelay) {
		...
		this.hover_delay = Number(aDelay); // Hover delay activation
